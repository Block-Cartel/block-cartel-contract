...
contract BLOCKSBondDepository is BLOCKSAccessControlled {
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 BLOCKS ( underflow protection )
    ...
    function maxPayout(uint256 _BID) public view returns (uint256) {
        return BLOCKS.totalSupply().mul(bonds[_BID].terms.maxPayout).div(100000);
    }
    ...
    */
    ...
    /**
     * @notice calculate current ratio of debt to BLOCKS supply
     * @param _BID uint
     * @return debtRatio_ uint
     */
    function debtRatio(uint256 _BID) public view returns (uint256 debtRatio_) {
        debtRatio_ = FixedPoint.fraction(currentDebt(_BID).mul(1e9), BLOCKS.totalSupply()).decode112with18().div(1e18);
    }
